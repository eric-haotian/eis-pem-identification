...
- **高维参数辨识与 Oxford PyBOP 对比基准 (High-Dimensional Benchmark vs. PyBOP)** (`benchmark_high_dim_vs_pybop.py`)：
  - 对比了在真实带噪和先验波动工况下的高维物理参数反演能力。
  - SEISModel 在 25 个工作工况下自动筛选并**同时辨识 31 个参数**，仅需 **2.51 秒**（7 次仿真），且参数平均恢复误差为 **0.96%**（低于 1%），表现出极高的恢复精度和卓越的效率。
  - 牛津大学的 PyBOP (26.3) 在 13 个参数（其物理上限）的拟合中，由于时域仿真计算缓慢且缺乏高维相关性屏蔽诊断，CMA-ES 算法运行了 **4.68 秒**（276 次仿真）后**完全数值发散**（平均参数误差高达 61 万%），证明了我们在高维物理参数估计上的决定性优势。
